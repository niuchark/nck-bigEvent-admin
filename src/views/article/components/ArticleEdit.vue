...
// 默认数据，即一组空数据，用于给添加文章时给表单清空，进行初始化
const defaultForm = {
  title: '', // 标题
  cate_id: '', // 分类id
...
// open调用后，可以打开抽屉
const editorRef = ref() // 获取富文本编辑器对象，下面要调用它的setHTML方法来清空内容
const open = async (row) => {
  ...
  if (row.id) {
    ...
    formModel.value = { ...defaultForm } // 基于默认的空数据，重置form数据
    // 这里重置了表单的数据，但是图片上传本地预览的img地址和富文本编辑器内容需要手动重置
    imgUrl.value = ''
